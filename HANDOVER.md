...
- ~~The migration chain cannot bootstrap a fresh database from scratch.~~ **Fixed in a follow-up pass** (`fix/migration-bootstrap`): the empty first migration now contains a reconstructed initial baseline, and the 3 migrations that referenced production-only data are now guarded to no-op safely elsewhere. Verified via a byte-for-byte schema diff against `schema.sql` and a full RLS-test pass against a genuinely migration-built database. See `supabase/tests/README.md`'s "Migration-bootstrap history" section.
...
| `supabase/tests/README.md` | How to run the real local-Postgres RLS integration test; the remaining Windows port-exclusion caveat; and the now-fixed migration-bootstrap history. |
...
| Migration-bootstrap fix (fresh database from `supabase/migrations/*.sql` alone) | **DONE** (follow-up pass, `fix/migration-bootstrap`) — verified via a byte-for-byte schema diff and a full RLS-test pass against a genuinely migration-built database; see `supabase/tests/README.md` |
...
With Docker and ports working, a **second, more significant finding** emerged: replaying `supabase/migrations/*.sql` against a genuinely empty database failed — `supabase/migrations/20260722092124_name_of_your_migration.sql`, the first migration chronologically, was an **empty file**, so no earlier migration ever created the base schema a later one (`20260724100000_add_missing_rls_policies.sql`) assumed existed. This was the concrete, now-confirmed consequence of a risk `docs/PROJECT_ROADMAP.md` already flagged generically ("at least one migration file was left empty with an unedited default name"). At the time this section was first written, this was left unfixed, pending explicit approval given the scope.

**Update — fixed in a dedicated follow-up pass (`fix/migration-bootstrap`).** A full audit of every migration found the empty first migration was not the only blocker: three more migrations (`20260724110000_seed_dev_workspace_membership.sql`, `20260725100000_phase9_test_second_workspace.sql`, `20260725150000_phase11_verification_workspaces.sql`) reference a specific profile id that only exists as real data on the linked production project, and would also abort a fresh replay with a foreign-key violation. Repair strategy: the empty migration now contains a reconstructed initial baseline schema, derived by applying `schema.sql` to a real local Postgres instance and then reversing — via real `DROP POLICY`/`ALTER TABLE DROP COLUMN`/`DROP FUNCTION` statements, not manual guessing — exactly the deltas that later migrations reintroduce, then dumping what remained with `supabase db dump` (the same tool that produced `schema.sql` itself). The three profile-dependent migrations were wrapped in `WHERE EXISTS(...)`/`IF EXISTS(...)` guards, changing their behavior only in the previously-impossible "fresh local database" case (a clean no-op there) with zero behavior change on the real linked project (the profile exists there, so the guard is always true). Verified: a completely fresh local database (destroyed Docker volume) now applies all 15 migrations with zero errors, and the resulting schema is byte-for-byte identical to applying `schema.sql` directly (`diff`, exit code 0, compared via `supabase db dump --local --schema public` on both). The RLS test below now passes against a genuinely migration-built database, not just the workaround described next.

**Workaround used while this was still open (historical, no longer necessary — kept for the record):** bootstrapped the local database by applying `schema.sql` — this repository's own verified, current baseline snapshot — directly via `psql` inside the local Postgres container, bypassing the then-broken migration replay entirely. This was still 100% real: the actual tables, the actual 19 `CREATE POLICY` statements, the actual `is_workspace_member()` function, nothing mocked.
