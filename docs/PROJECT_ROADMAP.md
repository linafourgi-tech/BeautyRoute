...
- **Dataset licensing (BR-FS-001).** The face-shape classifier's underlying training data is not cleared for use — status is **REQUIRES LICENSE REVIEW** per `docs/ai/FACE_SHAPE_DECISION.md`. This blocks all BR-FS-001 implementation work and has no committed resolution date; it depends on external parties (dataset creators/authors) responding to outreach. Definitive, cross-checked current status (stage-by-stage, dataset candidates, licensing, training-readiness checklist, exact next action): `docs/ai/BR-FS-001_CURRENT_STATUS.md`, added during the Phase 14 handover-closure pass.
- **Automated test coverage exists but is not exhaustive.** Phase 11 (Testing) is complete — 255 Vitest tests and 43 Deno tests now cover plan gating, auth/route guards, and the core domain pages, enforced in CI (current counts are higher; see `README.md`/`HANDOVER.md` for the up-to-date figure — this line preserves Phase 11's own historical count). What remains genuinely untested: RLS policy enforcement itself. **Updated during the Phase 14 handover-closure pass:** one real local-Postgres RLS integration test now exists and passes (`supabase/tests/rls_workspace_isolation.sql`, `npm run test:rls:local`, proving workspace isolation on the `clients` table) — this is a first real test, not a full integration tier; every other RLS-protected table is still only covered by mocked-boundary tests, and it is not wired into CI. End-to-end user-flow tests remain fully untested. See `docs/testing/PHASE_15_FINAL_REPORT.md`, `docs/quality/PHASE_16_REMEDIATION_REPORT.md`, and `supabase/tests/README.md` for the full breakdown.
...
- **2026-08-11 — Migration-bootstrap fix: fresh local databases now build correctly from `supabase/migrations/*.sql` alone.** Follow-up to the same-day handover-closure pass below, which first surfaced the empty-first-migration defect but didn't fix it. A full chronological audit of all 15 SQL migrations found the problem was larger than the one empty file: three more migrations (`20260724110000_seed_dev_workspace_membership.sql`, `20260725100000_phase9_test_second_workspace.sql`, `20260725150000_phase11_verification_workspaces.sql`) insert rows referencing a specific profile id that only exists as real data on the linked production project, and would also abort a fresh replay with a foreign-key violation. Repair, reconstructed empirically rather than hand-typed: applied `schema.sql` to a real local Postgres instance, then reversed — via real `DROP POLICY`/`ALTER TABLE DROP COLUMN`/`DROP FUNCTION` statements against that same database, not manual guesswork — exactly the deltas that migrations `20260724100000`, `20260724130000`, `20260803120000`, and `20260803140000` each reintroduce, and dumped the result with `supabase db dump` (the same tool that produced `schema.sql` itself) into the previously-empty `20260722092124_name_of_your_migration.sql`. The three profile-dependent migrations were wrapped in `WHERE EXISTS(...)`/`IF EXISTS(...)` guards — a deliberate, narrowly-scoped change to already-applied migration files, justified because it only changes behavior in the previously-impossible "fresh local database" case (a clean no-op) with zero behavior change on the real linked project (the referenced profile exists there, so the guard is always true and every insert proceeds exactly as before). No migration was deleted, no timestamp was changed, no data was seeded into the shared project. Verified: a completely fresh local database (Docker volume destroyed and recreated) applies all 15 migrations with zero errors; the resulting schema is byte-for-byte identical to applying `schema.sql` directly (`diff`, exit code 0, both sides dumped via `supabase db dump --local --schema public`); the real RLS integration test added in the handover-closure pass now passes against this genuinely migration-built database, not just the `schema.sql`-direct-apply workaround it used before. `HANDOVER.md`, `README.md`, and this document's own Risks section updated to reflect the fix; no historical entry altered. No application code, RLS policy, or CI configuration changed. Full record: `supabase/tests/README.md`'s "Migration-bootstrap history" section.
- **2026-08-11 — Handover-closure pass: BR-FS-001 status consolidated, one real RLS integration test added, no application code changed.** Re-audited BR-FS-001 from primary sources (all 6 existing `docs/ai/` documents, plus a repository-wide search — no hidden code or additional research material found) and published `docs/ai/BR-FS-001_CURRENT_STATUS.md`: a stage-by-stage status table (17 items, DONE/BLOCKED/NOT STARTED, each cited), an exact per-candidate dataset-licensing table (all 3 candidates — Roboflow CC BY 4.0 re-export of unverified provenance, Kaggle niten19 with a blank license field, and the Pasupa/Sunhem/Loo 2019 peer-reviewed academic lead requiring outreach that hasn't happened), a training-readiness checklist, an unsent draft permission-request template, and an explicit decision not to start training (no dataset's academic rights are yet verified). Separately, re-investigated real local-Postgres RLS integration testing (previously deferred for lacking Docker in a different environment) and implemented one: `supabase/tests/rls_workspace_isolation.sql` (run via `npm run test:rls:local`) proves the real `clients_access` policy enforces workspace isolation, against a real local Postgres instance with `schema.sql`'s actual RLS policies applied — no mocked client, no mocked RLS decision, self-cleaning (transaction `ROLLBACK`), fail-fast guarded to never reach anything but a local Docker container. Building it surfaced a real, previously-only-generically-flagged defect: `supabase/migrations/20260722092124_name_of_your_migration.sql` (the first migration) is empty, so the migration chain cannot bootstrap a database from scratch — worked around locally (`schema.sql` applied directly), not fixed (reconstructing an initial-schema migration needs explicit approval, out of scope here). `HANDOVER.md` and this document's own Risks section were updated to reflect both findings; no other historical entry in this log was altered. No application code, RLS policy, or CI configuration was changed. Full record: this entry, `docs/ai/BR-FS-001_CURRENT_STATUS.md`, `supabase/tests/README.md`.
